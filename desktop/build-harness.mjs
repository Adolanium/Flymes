...
await build({entryPoints:[resolve(here,'arena-harness.jsx')],bundle:true,format:'esm',outfile:resolve(here,'arena-harness.bundle.js'),nodePaths:process.env.FLYMES_NODE_MODULES?[process.env.FLYMES_NODE_MODULES]:[],plugins:[{name:'host-stub',setup(b){b.onResolve({filter:/^@hermes\/plugin-sdk$/},()=>({path:'host-stub',namespace:'harness'}));b.onLoad({filter:/.*/,namespace:'harness'},()=>({contents:'export const host = {}'}))}}]})
