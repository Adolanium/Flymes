...
The inspected sibling checkout and installed Hermes source both resolve to commit `ad03f20dd61919ca2135d6904e787a94284aacaf`. Development uses `E:/Dev/Hermes/hermes-agent/.venv/Scripts/python.exe`. The installed application's source lives under `%LOCALAPPDATA%/hermes/hermes-agent`; that installation does not have the same `.venv/Scripts/python.exe` path. Set `FLYMES_HERMES_PYTHON` and `FLYMES_HERMES_ROOT` explicitly when using a different installation.
...
The native agent plugin in `agent-plugin/` registers `flymes-complete` through documented `ctx.register_cli_command`, implemented in `hermes_cli/plugins.py` at line 638. Each completion child invokes `python -m hermes_cli.main flymes-complete`; Hermes discovers the enabled plugin, supplies its context, and the handler calls `ctx.llm.complete_structured`. Requests and results use stdin/stdout, with a tagged JSON result so unrelated host startup output cannot become a tool request. Flymes does not construct a private plugin context, edit Hermes source or instantiate `AIAgent`.

Install this package as `%HERMES_HOME%/plugins/flymes-worker` and enable it with `hermes plugins enable flymes-worker`. It registers no tools and requires no permission to replace built-in tools. This native agent package is separate from the Desktop panel package called `flymes`. The initial proof used a direct context bootstrap; that historical run remains labeled in `adapter-live-bootstrap-result.json`. The current implementation uses native CLI plugin loading.
...
This distinction means a full demonstration of native Hermes transcript turns executing simulator-selected tools remains unfulfilled. The implemented route demonstrates real Hermes-hosted completions controlling actual validated tools. It must not be described as replacing Hermes' standard agent orchestration or as a verified native conversation integration.
