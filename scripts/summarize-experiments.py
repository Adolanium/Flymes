...
for run_id in ids:
    ...
    source = ROOT/'.flymes/runs'/run_id/'replay.json'
    ...
    export_replay(source, ROOT/'artifacts'/f"replay-{run['mode'].lower()}.json")
report = {'kind':'exploratory closed-loop comparisons, one run per mode; not a statistical evaluation',
          ...
          'runs':rows}
atomic_json(ROOT/'artifacts/closed-loop-results.json',report)
